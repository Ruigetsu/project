...
def realtime_task():
    ...
    for row in balance.iterator():
        new_balance = get_balance(row.wallet_id.wallet_address, row.token_address, row.network_id.id)
        new_price = get_token_price(row.token)
        if new_balance != row.balance or new_price != row.price:
